...
@section('content')
    <h3 class="page-title">Problem List</h3>

    <p>
        <a href="{{ route('admin.problem.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a>
                    ...
                    @if (count($results) > 0)
                        ...
                        @foreach ($results as $user)
                            ...
                            <tr data-entry-id="{{ $user->id }}">
                                ...
                                <td>
                                    <a href="{{ route('admin.problem.edit',[$user->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    {!! Form::open(array(
                                        'style' => 'display: inline-block;',
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('".trans("quickadmin.qa_are_you_sure")."');",
                                        'route' => ['admin.problem.destroy', $user->id])) !!}
                                    {!! Form::submit(trans('quickadmin.qa_delete'), array('class' => 'btn btn-xs btn-danger')) !!}
                                    {!! Form::close() !!}

                                    @if($user->status != 1)
                                        <a href="{{url('admin/problems/active/'.$user->id)}}"
                                    ...
                                    @else
                                        <a href="{{url('admin/problems/inactive/'.$user->id)}}"
