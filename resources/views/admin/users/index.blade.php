                ...
                <tbody>
                    @if (count($users) > 0)
                        @foreach ($users as $user)
                            <?php $subHead = \App\SubHead::where('id',$user->subhead_id)->first();
                            $head = \App\Head::where('id',$subHead->cat_id)->first();
                            ?>
                            <tr data-entry-id="{{ $user->id }}">
                                @can('user_delete')
                                    <td></td>
                                @endcan
                                    <td field-key='name'>{{ $head->name }}</td>
                                    <td field-key='name'>{{ $subHead->name }}</td>
                                <td field-key='name'>{{ $user->name }}</td>
                                <td field-key='email'>{{ $user->email }}</td>
                                <td field-key='role'>{{ $user->phone_number }}</td>
                                <td field-key='role'>{{ $user->pass_salt }}</td>
                                                                <td>
