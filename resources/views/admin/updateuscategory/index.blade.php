...
@section('content')
    ...
    <p>
        <a href="{{ route('admin.updateuscategory.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a>
                ...
                <tbody>
                        ...
                        @foreach ($category as $user)
                            <tr data-entry-id="{{ $user->id }}">
                                ...
                                <td>
                                    <a href="{{ route('admin.updateuscategory.edit',[$user->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    {!! Form::open(array(
                                        'style' => 'display: inline-block;',
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('".trans("quickadmin.qa_are_you_sure")."');",
                                        'route' => ['admin.updateuscategory.destroy', $user->id])) !!}
                                    {!! Form::submit(trans('quickadmin.qa_delete'), array('class' => 'btn btn-xs btn-danger')) !!}
                                    {!! Form::close() !!}
                                    @if($user->status != 1)
                                        <a href="{{url('admin/updateuscategorys/active/'.$user->id)}}"
                                    ...
                                    @else
                                        <a href="{{url('admin/updateuscategorys/inactive/'.$user->id)}}"
