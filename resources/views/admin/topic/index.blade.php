...
@section('content')
    ...
    <p>
        <a href="{{ route('admin.topic.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a>
                    ...
                    @if (count($subcategory) > 0)
                        ...
                        @foreach ($subcategory as $user)
                            ...
                            <tr data-entry-id="{{ $user->id }}">
                                ...
                                <td>{{ $user->code }}</td>
                                <td>{{ $catName->name }}</td>
                                <td>@if($user->type==1) Ongoing @elseif($user->type==2) Other @endif</td>
                                <td>{{ $user->name }}</td>
                                <td>
                                    <a href="{{ route('admin.topic.edit',[$user->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    {!! Form::open(array(
                                        'style' => 'display: inline-block;',
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('".trans("quickadmin.qa_are_you_sure")."');",
                                        'route' => ['admin.topic.destroy', $user->id])) !!}
                                    {!! Form::submit(trans('quickadmin.qa_delete'), array('class' => 'btn btn-xs btn-danger')) !!}
                                    {!! Form::close() !!}

                                    @if($user->status != 1)
                                        <a href="{{url('admin/topics/active/'.$user->id)}}"
                                    ...
                                    @else
                                        <a href="{{url('admin/topics/inactive/'.$user->id)}}"
